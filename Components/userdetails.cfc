	...
	<cffunction name="uploadExcel" access="remote" returntype="string">		
		...
		<cfif local.excelData.recordCount GT 1>	
			...
			<cfloop query="local.excelData" startrow="2">
				...
				<cfelseif NOT(isValid("date", local.excelData.COL_6)) OR Len(local.excelData.COL_6) neq 10><!---OR (REFind(local.datepattern, local.slicedData.COL_6) EQ 0)--->
					<cfset local.errorFlag=1>
		...
		<cfset local.timestamp = DateFormat(now(), "yyyy-mm-dd_HH-MM-ss")>
		<cfset local.uniqueFilename = "ResultData_#local.timestamp#.xlsx">
